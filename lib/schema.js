...
export const registration = pgTable("Registration", {
	...
	sentDate: timestamp({ withTimezone: true, mode: 'string' }),	source: varchar({ length: 255 }),
	updatedAt: timestamp({ withTimezone: true, mode: 'string' }).defaultNow(),
	statusId: uuid().notNull(),
	documentTypeId: uuid().notNull(),
	summary: text(),
	fileStorageId: varchar({ length: 255 }),
}, (table) => [
	foreignKey({
			columns: [table.departmentId],
			foreignColumns: [department.id],
...
export const department = pgTable("Department", {
	...
	name: varchar({ length: 255 }).notNull(),
	description: text(),
	departmentStorageId: varchar({ length: 255 }), // id for department storage link
	createdAt: timestamp({ withTimezone: true, mode: 'string' }).defaultNow(),
	updatedAt: timestamp({ withTimezone: true, mode: 'string' }).defaultNow(),
}, (table) => [
...
export const register = pgTable("Register", {
	...
	registerStorageId: varchar({ length: 255 }), // id for register storage link
	createdAt: timestamp({ withTimezone: true, mode: 'string' }).defaultNow(),
	updatedAt: timestamp({ withTimezone: true, mode: 'string' }).defaultNow(),
});
...
// Funcție pentru a obține următorul număr de înregistrare
export async function getNextRegistrationNumber(registerId) {
	try {
		// Găsește ultimul număr de înregistrare pentru registrul specificat
		const lastRegistration = await db
			.select({
				registrationNo: registration.registrationNo
			})
			.from(registration)
			.where(eq(registration.registerId, registerId))
			.orderBy(desc(registration.registrationNo))
			.limit(1);

		if (lastRegistration.length === 0) {
			// Dacă nu există înregistrări, verifică dacă registrul are un minNumber configurat
			const registerData = await db
				.select({
					minNumber: register.minNumber
				})
				.from(register)
				.where(eq(register.id, registerId))
				.limit(1);

			// Returnează minNumber dacă este configurat, altfel începe de la 1
			return registerData[0]?.minNumber || 1;
		}

		// Returnează următorul număr disponibil
		return lastRegistration[0].registrationNo + 1;
	} catch (error) {
		console.error('Eroare la obținerea următorului număr de înregistrare:', error);
		throw new Error('Nu s-a putut obține următorul număr de înregistrare');
	}
}