  ...
  try {
    ...
    try {
      await axios.post(`${process.env.N8N_URL}/webhook/registries`, {
        action: "update",
        registerId: id,
        registerStorageId: updated[0].registerStorageId,
        oldName: body.oldName || updated[0].name, // fallback dacă nu e trimis explicit
        newName: name,
        departmentId: updated[0].departmentId
      });
    } catch (webhookError) {
      console.error("n8n webhook error (registry update):", webhookError?.message || webhookError);
    }
...
export async function DELETE(req, context) {
  try {
    const { id } = await context.params;    if (!id) return Response.json({ error: 'ID lipsă' }, { status: 400 });
    ...
    try {
      await axios.post(`${process.env.N8N_URL}/webhook/registries`, {
        action: "delete",
        registerId: id,
        registerStorageId: deleted[0].registerStorageId,
        name: deleted[0].name,
        departmentId: deleted[0].departmentId
      });
    } catch (webhookError) {
      console.error("n8n webhook error (registry delete):", webhookError?.message || webhookError);
    }
